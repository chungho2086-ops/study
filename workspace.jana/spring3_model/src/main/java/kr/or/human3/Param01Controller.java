...
//			request.getRequestDispatcher("/WEB-INF/views/join.jsp2").forward(request, response);
	...
	@RequestMapping("/join4.do") // 4번을 가장 많이 씀 90% 이거를 많이 씀
...
//	public String join3(String id, String pw, String tel) {
	...
	public ModelAndView singup2(
			HttpServletRequest request,
			
			String id
			) {
		System.out.println("/singup2: 실행");
		System.out.println("id: "+ id);
...
//		request.setAttribute("id", id);
	...
	public ModelAndView singup3(
			// parameter에서 꺼내서 
			// DTO의 setter를 이용하ㅕ여 자동으로 넣어줌
			@ModelAttribute
			MemberDTO dto1,
...
//			@ModelAttribute("memberDTO")
	...
	@RequestMapping("/singup4")
	public ModelAndView singup4(MemberDTO dto1, String id) {
		System.out.println("/sinhup4: 실행");
	...
	@RequestMapping("/cal/{moth}")
	public void cal2(
			@PathVariable("moth") // 생략 불가능
			int month	
	) {
		System.out.println(month + "월 달력입니다");
	}
	@RequestMapping("lunch/{store}/prder/{menu}/start")
	public void lunch(
			@PathVariable("store")
			String store,
			
			@PathVariable("menu") // 변수명이 같으면 () 생략 가능
			String menu
